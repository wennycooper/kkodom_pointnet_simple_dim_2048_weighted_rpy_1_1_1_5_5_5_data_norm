...
#def plot_trajectory(positions, absolute_poses, show_axes=False):
...
def gen_12dim_tsv(absolute_poses, tsv_file):

    with open(tsv_file, 'w') as f:
        writer = csv.writer(f, delimiter = ' ')
        for pose in absolute_poses:
            row = pose[:3].flatten().tolist()
            writer.writerow(row)

# Example Usage
if __name__ == "__main__":
    # Replace with your actual KITTI dataset path
    kitti_root = './inference_output'
    # data_list = load_kitti_odometry_data(kitti_root)

    now = dt.now().strftime("%Y-%m-%d-%H-%M-%S")

    #kitti_root = '/home/its/kkodom/kitti_dataset_6dim_seq_00_only'
    ...
    tsv_file = f'{sequence_id:02d}.txt'

    gen_12dim_tsv(absolute_poses, tsv_file)
    print("Output to", tsv_file)
    print("Please use kitti_odom_eval to check it")
